...
class AlipaySubmit {
	...
	/**
	 * 生成签名结果
	 * @param $para_sort 已排序要签名的数组合并好后的字符串
	 * return 签名结果字符串
	 */
	...
	function buildRequestParaToString($para_temp) {
		...
		$request_data = createLinkstringQuotation($para);

		//生成签名结果
		$mysign = $this->buildRequestMysign($request_data);
		//签名结果与签名方式加入请求提交参数组中
		$request_data .= '&sign='."\"".urlencode($mysign)."\"&sign_type=\"".strtoupper(trim($this->alipay_config['sign_type']))."\"";
	...
	function buildRequestForm($para_temp, $method, $button_name) {
		...
		//生成签名结果
		$mysign = $this->buildRequestMysign($prestr);

		//签名结果与签名方式加入请求提交参数组中
		$para['sign'] = $mysign;
		$para['sign_type'] = strtoupper(trim($this->alipay_config['sign_type']));
